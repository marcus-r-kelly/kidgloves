...
class LARGeSSE_G(object) : 
    ...
    def _assign_hierarchy(self,hierarchy,system_limit_upper=2000,system_limit_lower=4) : 
        ...
        # filter redundancy 
        hks=list(self.hierarchy.keys())
        topop=set()
        for i in range(len(hks)-1) : 
            for j in range(i+1,len(hks)) : 
                if self.hierarchy[hks[i]] == self.hierarchy[hks[j]]  : 
                    topop.add(hks[j])
        for tp in topop :
            self.hierarchy.pop(tp)

        # figure out which genes are already in this lg
        ...
        for j,sk in enumerate(ssk) : 
            #map things to row *IF* they are already in the hierarchy
        ...
        with warnings.catch_warnings() : 
            warnings.simplefilter('ignore')
            # below line triggers a UserWarning about the beta states of CSRs
            self.nmo=torch.sparse.mm(self.ii,sphier)

        self.systems=ssk

    #def _assign_omics(self,omics,resolve_cn_conflicts=True) : 
    def _assign_omics(self,omics) : 
        """
        omics should be a pd.DataFrame as generated by cohort_preprocessing.compile_omics
        """
        ...
        self.full_index=np.array(
                [ e+'_mut' for e in self.genes ]+[ c for c in omics.columns if not c.endswith('_mut') ]
                )

        self.omics=_fix_overlong_identifiers(omics).reindex(columns=self.full_index).fillna(0).astype(np.float32)
        ltypes=np.vectorize(lambda s : s.split('_')[-1])(self.omics.columns)

        self.t_omics=torch.tensor(self.omics.values,
                                    dtype=DTYPE,
                                    device=self.device)

        self._c_omics_mut=torch.tensor((ltypes == 'mut') ,device=CPU)
        self._c_omics_fus=torch.tensor((ltypes == 'fus'),device=CPU)
        self._c_omics_up=torch.tensor(( ltypes == 'up'),device=CPU)
        self._c_omics_dn=torch.tensor(( ltypes == 'dn'),device=CPU)
    ...
    def guess_weights(self) : 
        ...
        with warnings.catch_warnings() : 
            ...
            #ll=Lasso(alpha=5e-4)
            ll=LinearRegression()
            csr=scipy.sparse.csr_matrix((self.X.values().cpu().numpy(),self.X.indices().cpu().numpy()),shape=np.array(self.X.shape))
            ll.fit(csr,np.log((self.y+1)/self.omics.shape[0]))
            guesses_ll=torch.tensor(np.clip(ll.coef_-np.median(ll.coef_[ ll.coef_ > 0 ]),0,np.inf)
                                     ,device=self.device)
            guess_intercept_ll=torch.tensor(ll.intercept_,device=self.device)

        guesses_cc=torch.clip(cc2tens(self.X,self.t_y),0,torch.inf).ravel()
        guess_intercept_cc=torch.tensor(-5,device=DEVICE,dtype=torch.float)

        guesses=torch.stack([guesses_ll,guesses_cc],dim=0).max(axis=0).values

        guess_intercept=torch.stack([guess_intercept_ll,guess_intercept_cc]).min()
...
class OptimizerResult : 
    ...
    val_criterion : npt.NDArray[typing.Any]
    strengths : list 
    masks : tuple 
    ...
    def __call__(self,epoch,tX,ky,n) : 
        ...
        if epoch in self.sampling_epochs: 
            ...
            if self.se > self.sampling_epochs_to_check_convergence and self.check_convergence : 
                ...
                # this loss is greater than or equal to(=worse) than this many of the previous epochs
...
def aic_path(lg,wpath,ipath,patient_indices=None,global_feature_mask=None) : 
    
    if not torch.is_tensor(wpath) : 
        wpath=torch.tensor(wpath,device=lg.device)
        
    if not torch.is_tensor(ipath) : 
        ipath=torch.tensor(ipath,device=lg.device)

    if global_feature_mask is not None : 
        if torch.is_tensor(global_feature_mask) : 
            tX=mask_sparse_columns(lg.X.clone(),global_feature_mask).coalesce()
        else : 
            tX=mask_sparse_columns(lg.X.clone(),torch.tensor(global_feature_mask,device=lg.device,dtype=torch.bool)).coalesce()
    else : 
        tX=lg.X
        
    if patient_indices is not None and not torch.is_tensor(patient_indices) : 
        patient_indices=torch.tensor(patient_indices,device=lg.device)
        
    eps=torch.finfo(wpath.dtype).eps
    # multibetas is genes x epochs

    if wpath.shape[0] == lg.X.shape[1] : 
        phat=torch.clip(
            torch.special.expit(torch.matmul(tX,wpath)+ipath),
            eps,
            1-eps
        )
    else : 
        phat=torch.clip(
            torch.special.expit(torch.matmul(tX,wpath.transpose(0,1))+ipath),
            eps,
            1-eps
        )
    ...
    if patient_indices is None : 
        ks=lg.t_y
        n=torch.tensor(lg.patients.shape[0],device=lg.device)
    else :
        ks=lg.t_omics[patient_indices].sum(axis=0).reshape(-1,1)
        #n=torch.tensor(patient_indices.shape[0],device=lg.device)
        n=patient_indices.shape[0]
        
    comb=(torch.lgamma(torch.tensor(n+1,device=lg.device))-torch.lgamma(ks+1)-torch.lgamma(n-ks+1))
    indiv_ells=torch.log(phat)*ks+torch.log(1-phat)*(n-ks)
    llterm=(comb+indiv_ells).sum(axis=0)
    kpenalty=(wpath > 0).sum(axis=1)

    modpenaltydenom=torch.stack([lg.t_y.shape[0]-1-kpenalty,
                                lg.t_y.shape[0]/kpenalty],axis=0).max(axis=0).values



    criteria= 2*kpenalty - 2*llterm + 2*kpenalty*(kpenalty+1)/modpenaltydenom
...
def wrap_spoof(lg,settings,strengths,spdir,preserve_rowsums) : 
    os.makedirs(spdir,exist_ok=True)
    sh=spoof(lg.hierarchy,preserve_rowsums=preserve_rowsums)

    lgspoof=assemble_lg(settings,omics=lg.omics,msig=lg.signatures,lengths=lg.lengths,timings=lg.timings)
    ...
    spoof_kws.update(dict(savefile=opj(spdir,'logger.pt')))
    torch.save(X,opj(spdir,'spX.pt'))
    ...
    return splog.convergence_status

def spoof_gen(lg,settings,strengths,preserve_rowsums=[True,False,]) : 
    for pref,tf in zip(['fake','wild'],[True,False]) : 
        for x in range(settings.n_spoofs) : 
            spdir=opj(settings.output_directory,pref+'_'+str(x))
            yield wrap_spoof(lg,settings,strengths,spdir,preserve_rowsums)
...
def run_model(lg,
    ...
    if global_feature_mask is not None : 
        if not torch.is_tensor(global_feature_mask) : 
            global_feature_mask=torch.tensor(global_feature_mask,device=DEVICE,dtype=torch.bool)
    ...
    #TODO : you need to cancel early stopping but instead examine the _criterion_ 
    # of the outgroups

    if monitor : waitbar=tqdm(total=max_epochs) 
    ...
    for epoch in range(max_epochs): 
        ...
        #240229 -- switched to training stuf
        if convergence_status := logger(epoch,tX,ytr,ntr) : break

        if monitor: waitbar.update()
    ...
    if monitor : 
        if convergence_status : 
            waitbar.update(max_epochs-epoch) 
        waitbar.close()


    ose,llog,wlog,ilog=logger.wrap()
    val_criterion=aic_path(lg,wlog,ilog,patient_indices_test,global_feature_mask=global_feature_mask).cpu().numpy()
...
def optimize_regstrength_quick(
        lg,
        globalmask,
        masks,
        strengths,
    ...
    if torch.is_tensor(masks[0]) :
        npmasks=[ m.cpu().numpy() for m in masks ]
    else: 
        npmasks=masks

    try : 
        for hme in range(n_passes) :
            shift=2*(5/8)**(hme)
            #mestrrange=np.linspace(nominal_best-range_amp,nominal_best+range_amp,n_points_per_pass)
            ...
            for e in range(n_folds) : 
                for i in range(2) : 
                    ...
                    min_val_criterion=optres.val_criterion.min()
                    best_epoch_this=optres.val_criterion.argmin()
                    ...
                    last=pd.Series(dict(
                                       ...
                                       criterion=min_val_criterion,
                                       elapsed=int(time.time()-starttime//1.0),
                                       nparams=nparams,
                                       convergence_status=optres.convergence_status,
                                       sumweights=best_weights_this.sum(),
                                       lynp=max(ynp.shape),
                                       effective_penalty=sum([ np.exp(s)*max(ynp.shape)*best_weights_this[m].sum() for s,m in zip(strengths,npmasks) ]),
                                   ))

                    last.name='{: >2}:{: >2}:{: >1}'.format(hme,e,i)
                    ...
                    if i == 1 : # only change direction after the second paired run of each fold is done
                        if loggertable.iloc[-2].criterion < (loggertable.iloc[-1].criterion-1) :
                            # things are headed in the wrong direction
                            shift_positive=(not shift_positive)
                            # switch the shift direction from whatever it was
                    if i ==0  : # apply the shift only after the first paired run of each fold is done
                        if shift_positive :  
                            strengths[tweakwhichstrength] += shift
                        else : 
                            strengths[tweakwhichstrength] -= shift
                        
                if (loggertable.tail(2*n_folds).nparams == 0).all() : break #NEW
    finally: 
    ...
    for hme in range(n_passes) :
        ...
        for metaepoch,strength_this_me in enumerate(mestrrange) : 
            ...
            for e in range(n_folds) : 
                ...
                last=pd.Series(dict(
                                   metaepoch=hme*n_points_per_pass+metaepoch,
                                   fold=int(e),
                                   regstrength=strengths[tweakwhichstrength],
                                   criterion=min_val_criterion,
                                   elapsed=int(time.time()-starttime//1.0),
            ...
            if (loggertable.tail(n_folds).nparams == 0).all() : break #NEW
...
def postprocess_burnintable(burnintable) :
    ...
    with warnings.catch_warnings() : 
        warnings.simplefilter('ignore')
        # below lines trigger an inoperable FutureWarning about underlying functions
        cfmax=burnintable[ burnintable.convergence_status ].groupby('fold').criterion.max()
        cfmin=burnintable[ burnintable.convergence_status ].groupby('fold').criterion.min()
...
def estimate_null_regstrength(lg,settings,j_regstrength,save=False): 
    ...
    burnintable,ofinterest_genes=optimize_regstrength_quick(
    #burnintable,ofinterest_nogenes=optimize_regstrength_gradient(
                                        lg,
                                        globalmask=notasystem,
    ...
    bests=burnintable.loc[burnintable.groupby('fold').criterion.idxmin()]

    consensus_regstrength=bests.regstrength.mean()

    if save : 
        burnintable.to_csv(opj(settings.output_directory,'null_burn_in.csv'))

    return (consensus_regstrength,burnintable)
...
def do_burn_in(lg,settings,savefile=None) : 
    ...
    burnintable,ofinterest_nogenes=optimize_regstrength_quick(
    #burnintable,ofinterest_nogenes=optimize_regstrength_gradient(
                                        lg,
                                        globalmask=notagene,
...
def assemble_lg(settings,omics=None,msig=None,lengths=None,timings=None) : 

    if omics is None or msig is None : 
        nuomics,numsig=prep_guts(settings.omics_path,settings.mutation_signature_path)
        if omics is None : 
            omics=nuomics
        if msig is None : 
            msig=numsig

    if lengths is None or timings is None : 
        nulengths,nutimings=load_protein_datas()
        if lengths is None : 
            lengths=nulengths
        if timings is None : 
            timings=nutimings
    ...
    #IMPORTANT!!!!ONE!!!
    genesinomics=np.unique(np.vectorize(lambda s : s.split('_')[0])(omics.columns))
    genesinhier=np.unique([ s for v in hier.values() for s in v ])
    wgenes=np.union1d(genesinomics,genesinhier)
    ocols=[ w+'_mut' for w in wgenes ]+list(omics.columns[~omics.columns.str.endswith('_mut')])
    omics=omics.reindex(columns=ocols).fillna(0)
...
def estimate_sigonly_regstrength(lg,settings,save=True) : 
    ...
    burnintable_sigonly,ofinterest_sigonly=optimize_regstrength_quick(
                                        lg,
                                        globalmask=isasig,
                                        masks=(isasig[isasig],),
    ...
    bests=burnintable_sigonly.loc[burnintable_sigonly.groupby('fold').criterion.idxmin()]
    ...
    return (consensus_regstrength,burnintable_sigonly)
...
def main_script_process(settings): 
    ...
    if len(lg.hierarchy) < 1 : 
        msg('Detected null hierarchy; optimizing hyperparameters.')
        msg('Optimizing regularization strength on the signature-only model...')
        consensus_regstrength,so_burninframe=estimate_sigonly_regstrength(lg,settings)  
        msg('Optimizing regularization strength on the gene-and-signature model...')
        null_regstrength,so_burninframe=estimate_null_regstrength(lg,settings,consensus_regstrength,save=True)
        torch.save([null_regstrength,consensus_regstrength],opj(settings.output_directory,'optimized_strengths.pt'))
        msg('All done.')
    else : 

        msg('Running full main model...')
        main_kws=compile_kwargs(lg,settings,which='main')
        mainout=run_model(lg,strengths=strengths,**main_kws) 
        
        msg('Running full matched null model...')
        null_kws=compile_kwargs(lg,settings,which='null')
        matchnullout=run_model(lg,strengths=strengths,**null_kws)
        ...
        sg=spoof_gen(lg,settings,strengths)
        [ spc for spc in tqdm(sg,total=2*settings.n_spoofs) ] ;
...
def legacy_run_model(lg,
    ...
    optimizer=torch.optim.LBFGS(model.parameters(),lr=lr,**lgbfs_kwargs)
    sampling_epochs=np.cast['int'](np.linspace(0,max_epochs-1,n_sampling_epochs)//1.0)
    logger=Logger(model,losser,sampling_epochs,tX.shape[1],check_convergence=check_convergence)
